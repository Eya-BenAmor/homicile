<?php
 ...
 require_once('C:\wamp64\www\PHP\ici\eya\Model\rdv.php');
class rdvC
{
...
function afficherrdv($id)
	{
		...
		function afficherrdvG($idp,$idm,$date,$etat)
	{
        $sql = "SELECT * FROM `rendez-vous` WHERE idp='$idp' && idm='$idm'  && dateti ='$date' &&  etat ='$etat'";
        $db = config::getConnexion();
        try {
			$liste = $db->query($sql);
			return $liste;
