...
if (isset($_POST['signup-submit'])) {
    ...
    if (empty($nom) || empty($prenom) || empty($email) || empty($psw) || empty($pswR) || empty($datenaissance) || empty($genre)) {
        header("Location: ../enregistrerC.php?error=emtyfields&nom=".$nom."&prénom=".$prenom."&email=".$email);
        exit();
    }
    elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {//CHECK IF THE EMAIL IS VALID
        header("Location: ../enregistrerC.php?error=invalidemail&nom=".$nom);
        exit();
    }
    elseif ($psw!==$pswR) {//check if the paswords matchs
        header("Location: ../enregistrerC.php?error=invalidpassword&nom=".$nom);
        exit();
    }
        ...
        if (!mysqli_stmt_prepare($statment, $sql)) { //cheking if our connection to the databse doesn't work
            header("Location: ../enregistrerC.php?error=sqlerror1");
            exit();
        }
        else{
            ...
            if ($resultcheck > 0){
                header("Location: ../enregistrerC.php?error=emailused");
                exit();
            }
            else {
                $sql = "INSERT INTO patient(nom, prenom, email, mdp, datenaissance, genre) VALUES(?,?,?,?,?,?)";
                ...
                if (! mysqli_stmt_prepare( $statment, $sql)) { //cheking if our connection to the databse works
                    header("Location: ../enregistrerC.php?error=sqlerror2");
                    exit();
                }
                else{
                    mysqli_stmt_bind_param($statment, "ssssss",$nom,$prenom,$email,$psw,$datenaissance,$genre);
                    mysqli_stmt_execute($statment);
                    header("Location: ../connecterC.php?signup=success");
                    exit();
                }
        }
    }
    ...
    header("Location: ../enregistrerC.php");
    exit();
}
